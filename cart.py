...
abalone_target = 'Sex'
abalone_acc = cart(abalone_path, abalone_target)

# database adult

adult_path = './csv/adult.csv'
adult_target = 'income'
adult_acc = cart(adult_path, adult_target)

# database australian

australian_path = './csv/australian.csv'
australian_target = 'A15'
australian_acc = cart(australian_path, australian_target)

# database drugs

drugs_path = './csv/drugs.csv'
drugs_target = 'Alcohol'
drugs_acc = cart(drugs_path, drugs_target)

# database fertility

fertility_path = './csv/fertility.csv'
fertility_target = 'Output'
fertility_acc = cart(fertility_path, fertility_target)

# database german

german_path = './csv/german.csv'
german_target = 'A25'
german_acc = cart(german_path, german_target)

# database glass

glass_path = './csv/glass.csv'
glass_target = 'Type'
glass_acc = cart(glass_path, glass_target)

# database heart

heart_path = './csv/heart.csv'
heart_target = 'A14'
heart_acc = cart(heart_path, heart_target)

# database ionosphere

ionosphere_path = './csv/ionosphere.csv'
ionosphere_target = 'A35'
ionosphere_acc = cart(ionosphere_path, ionosphere_target)

# database pendigits

pendigits_path = './csv/pendigits.csv'
pendigits_target = 'A17'
pendigits_acc = cart(pendigits_path, pendigits_target)

# database phishing

phishing_path = './csv/phishing.csv'
phishing_target = 'A31'
phishing_acc = cart(phishing_path, phishing_target)

# database failures

failures_path = './csv/failures.csv'
failures_target = 'outcome'
failures_acc = cart(failures_path, failures_target)

# database shuttle

shuttle_path = './csv/shuttle.csv'
shuttle_target = 'A10'
shuttle_acc = cart(shuttle_path, shuttle_target)

# database spam

spam_path = './csv/spam.csv'
spam_target = 'A58'
spam_acc = cart(spam_path, spam_target)

# database wdbc

wdbc_path = './csv/wdbc.csv'
wdbc_target = 'Cancer'
wdbc_acc = cart(wdbc_path, wdbc_target)

# database wifi

wifi_path = './csv/wifi.csv'
wifi_target = 'A8'
wifi_acc = cart(wifi_path, wifi_target)

# database wine

wine_path = './csv/wine.csv'
wine_target = 'Class'
wine_acc = cart(wine_path, wine_target)

# database zoo

zoo_path = './csv/zoo.csv'
zoo_target = 'type'
zoo_acc = cart(zoo_path, zoo_target)

# database breast

breast_path = './csv/breast.csv'
breast_target = 'Class'
breast_acc = cart(breast_path, breast_target)

# database stability

stability_path = './csv/stability.csv'
stability_target = 'stabf'
stability_acc = cart(stability_path, stability_target)
...
student_acc = cart(student_path, student_target)

# database leaf

leaf_path = './csv/leaf.csv'
leaf_target = 'Class'
leaf_acc = cart(leaf_path, leaf_target)
